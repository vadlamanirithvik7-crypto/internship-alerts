...
`python poller/main.py --board-slice 250 --resolve-slice 0 --verbose` runs ingestion and alerts. Employer boards run first with 12 concurrent fetches and commit every 10 completed companies. Half the rotating slice is reserved for the oldest company checks, preventing employers with existing matches from consuming every slot. Priority companies are additional to the slice. Separate feeds run with four concurrent fetches and commit independently as they finish. Feed fingerprints skip unchanged ingestion. Boards are learned from US engineering internships across seasons so employers can be watched before their Summer 2027 openings appear.

The free GitHub schedule requests checks every five minutes, offset from the top of the hour. Actual recent starts were hours apart despite scans completing in minutes; this schedule is not a five-minute delivery guarantee. An external scheduler can dispatch the existing GitHub workflow without moving database or email credentials out of GitHub. No paid services are required by the scanner changes.

`--discovery` adds SEC discovery. `--skip-search` skips broad keyword sources. HN and Reddit run at most every six hours; Amazon and Microsoft are due after 15 minutes; broad search is hourly. Tracker repository discovery runs after employer scans, at most every six hours, and its discovered feed URLs are cached and reused. `--board-slice 0` disables all board polling, including priority companies.
...
- **Isolation:** workers receive plain company snapshots instead of sharing ORM objects. Board progress commits every 10 companies. PostgreSQL advisory locks or a local file lock prevent overlapping pollers; CI also uses a concurrency group.
...
Original application links are preferred when a tracker provides both an aggregator and employer URL. Greenhouse, Lever, Workday and other ATS APIs are employer publishing systems; their returned job URL can be the company’s own careers page or its official hosted application. An aggregator-only URL is labeled “View source listing” instead of being presented as an employer application. Missing descriptions remain visible for manual review. Distinct requisition query parameters (such as `gh_jid` and `jobId`) are retained in deduplication, while tracking parameters are removed. Existing posting IDs, notes and application history survive the versioned eligibility/identity backfill.
